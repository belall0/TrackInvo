...
const fetchLatestInvoices = async () => {
  try {
    const data = await sql<definitions.LatestInvoiceRaw[]>`
...
const fetchFilteredInvoices = async (query: string, currentPage: number) => {
  ...
  try {
    const invoices = await sql<definitions.InvoicesTable[]>`
      SELECT
        invoices.id,
        invoices.amount,
        invoices.date,
...
const fetchInvoiceById = async (id: string) => {
  try {
    const data = await sql<definitions.InvoiceForm[]>`
      SELECT
        invoices.id,
...
const fetchCustomers = async () => {
  try {
    const customers = await sql<definitions.CustomerField[]>`
      SELECT
        id,
        name
      FROM customers
...
const fetchFilteredCustomers = async (query: string) => {
  try {
    const data = await sql<definitions.CustomersTableType[]>`
		SELECT
		  customers.id,
		  customers.name,
		  customers.email,
